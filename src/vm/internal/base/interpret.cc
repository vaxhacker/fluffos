 ...
 * UTF-8 bytes; an array must hold ints 0..255 (one byte each) and errors
 * on any other item. Anything else is illegal. Shared by buffer range
 * assignment, buffer + / +=, and the to_buffer() efun (which is also the
 * compile-time promotion for 'buffer b = str'). The caller owns (and
 * must free) the result. */
buffer_t* svalue_to_buffer_bytes(svalue_t* from) {
  if (from->type == T_STRING) {
    int len = SVALUE_STRLEN(from);
    buffer_t* b = allocate_buffer(len);
    memcpy(b->item, from->u.string, len);
    return b;
  }
  if (from->type == T_ARRAY) {
    ...
    /* validate before allocating so an error can't leak the buffer */
    for (int i = 0; i < n; i++) {
    ...
    buffer_t* b = allocate_buffer(n);
    for (int i = 0; i < n; i++) {
      b->item[i] = static_cast<unsigned char>(arr->item[i].u.number);
    }
    return b;
  }
...
void copy_lvalue_range(svalue_t* lval, svalue_t* from) {
  ...
  switch (owner->type) {
    ...
    case T_BUFFER: {
      ...
      buffer_t* fbuf =
          (from->type == T_BUFFER) ? from->u.buf : (conv = svalue_to_buffer_bytes(from));
      ...
      if (conv) {
        free_buffer(conv);
        free_svalue(from, "copy_lvalue_range: buffer rhs"); /* consume the string/array */
      } else {
...
void assign_lvalue_range(svalue_t* lval, svalue_t* from) {
  ...
  switch (owner->type) {
    ...
    case T_BUFFER: {
      ...
      /* from is NOT consumed here -- it remains the assignment's rvalue */
      if (conv) {
        free_buffer(conv);
      }
      break;
    ...
    switch (instruction) {
      ...
      case F_MAKE_REF: {
        ...
        if (op != F_GLOBAL_LVALUE && op != F_LOCAL_LVALUE && op != F_REF_LVALUE) {
          ...
          if (lv_owner_type == T_STRING) {
            ...
            md_record_ref_journal(PTR_TO_NODET(ref->sv.u.string), true, MSTR_REF(ref->sv.u.string),
      ...
      case F_ADD: {
        switch (sp->type) {
          ...
          case T_ARRAY: {
            if ((sp - 1)->type == T_BUFFER) {
              /* buffer + array of ints 0..255 appends the bytes */
          ...
          case T_STRING: {
            switch ((sp - 1)->type) {
              ...
              case T_BUFFER: {
                /* buffer + string appends the string's raw UTF-8 bytes */
                buffer_t* conv = svalue_to_buffer_bytes(sp);
                buffer_t* b = allocate_buffer((sp - 1)->u.buf->size + conv->size);
      ...
      case F_FOREACH: {
        ...
        if (flags & FOREACH_MAPPING) {
          ...
          sp->type = T_NUMBER;
          sp->subtype = 0;
          sp->u.number = 0; /* this loop's byte cursor */
        } else {
        ...
        if (flags & FOREACH_RIGHT_GLOBAL) {
          ...
          if (loc->type != T_NUMBER && loc->type != T_REF)
            error(
                ...
                "into scope.");
          if (loc->type == T_REF) {
            /* stale ref from a previous run of this same loop (e.g. an
             * inner loop re-entered by an outer one) -- release it or the
             * ref_t leaks */
        ...
        if ((sp - 1)->type == T_LVALUE) {
          ...
          if (idx < owner->u.buf->size) {
            if (sp->type == T_REF) {
              ...
              r->sv.u.lvalue_byte = owner->u.buf->item + idx;
              r->lvalue = &r->sv;
            } else {
      ...
      case F_TEMPLATE_COERCE: {
        /* Coerce top-of-stack value to string for template literal
         * interpolation. Strings pass through as-is; ints print in decimal;
         * floats use %g (strips trailing zeros); anything else falls back
         * to the same LPC literal representation sprintf's %O uses. */
        switch (sp->type) {
          case T_STRING:
            break;
          case T_NUMBER: {
            ...
            sp->type = T_STRING;
            sp->subtype = STRING_MALLOC;
            sp->u.string = string_copy(buff, "F_TEMPLATE_COERCE");
            break;
          }
          case T_REAL: {
            char buff[400];
          ...
          default: {
            ...
            sp->type = T_STRING;
            sp->subtype = STRING_MALLOC;
            sp->u.string = string_copy(outbuf.buffer ? outbuf.buffer : "", "F_TEMPLATE_COERCE");
            if (outbuf.buffer) {
              FREE(outbuf.buffer);
            }
            break;
  ...
  for (number_of_matches = 0; num_arg >= 0; number_of_matches++) {
    ...
    if (*fmt++ == '%') {
      ...
      switch (fmt[-1]) {
        ...
        case 'f': {
          LPC_FLOAT tmp_num = strtod(const_cast<char*>(in_string), const_cast<char**>(&in_string));
          if (!skipme2) {
            SSCANF_ASSIGN_SVALUE(T_REAL, u.real, tmp_num);
          }
          continue;
